        ...
        for l in range(0, self.num_layers):
            if self.name == "gcn-align":
                self.gnn_layers.append(
                    GCNAlign_GCNConv(
                        in_channels=self.hiddens[l], out_channels=self.hiddens[l+1], improved=False, cached=True, bias=bias)
                )
            elif self.name == "naea":
                self.gnn_layers.append(
                    NAEA_GATConv(in_channels=self.hiddens[l]*self.heads[l-1], out_channels=self.hiddens[l+1],
                                 heads=self.heads[l], concat=True, negative_slope=negative_slope, dropout=attn_drop, bias=bias)
                )
            elif self.name == "kecg":
                self.gnn_layers.append(
                    KECG_GATConv(in_channels=self.hiddens[l]*self.heads[l-1], out_channels=self.hiddens[l+1],
                                 heads=self.heads[l], concat=False, negative_slope=negative_slope, dropout=attn_drop, bias=bias)
                )
...
class Decoder(torch.nn.Module):
    def __init__(self, name, params):
        ...
        if self.name == "align":
            ...
            self.func = AlignEA(
                p=p, feat_drop=self.feat_drop, params=self.margin)
        elif self.name == "transedge":
            self.func = TransEdge(
                p=p, feat_drop=self.feat_drop, dim=params["dim"], mode="cp", params=self.margin)
        elif self.name == "mmea":
            self.func = MMEA(feat_drop=self.feat_drop)
        elif self.name == "transe":
            self.func = TransE(p=p, feat_drop=self.feat_drop,
                               transe_sp=transe_sp)
            ...
            self.func = RotatE(p=p, feat_drop=self.feat_drop,
                               dim=params["dim"], params=self.margin)
        elif self.name == "hake":
            self.func = HAKE(p=p, feat_drop=self.feat_drop,
                             dim=params["dim"], params=self.margin)
        elif self.name == "conve":
            self.func = ConvE(feat_drop=self.feat_drop,
                              dim=params["dim"], e_num=params["e_num"])
        # elif self.name == "SLEF-DESIGN":
        ...
        #     self.sampling_method = SLEF-DESIGN_sampling
            ...
            raise NotImplementedError(
                "bad sampling method: " + self.sampling_method)
...
class MTransE_Align(torch.nn.Module):
    ...
    def forward(self, e1, e2):
        if self.mode == "sa1":
            ...
            pred = - \
                torch.norm(torch.matmul(e1, self.weight) - e2, p=self.p, dim=1)
        else:
            raise NotImplementedError
        return pred
    ...
    def only_pos_loss(self, e1, e2):
        if self.p == 1:
            map_loss = torch.sum(
                torch.abs(torch.matmul(e1, self.weight) - e2), dim=1).sum()
        else:
            map_loss = torch.sum(torch.pow(torch.matmul(
                e1, self.weight) - e2, 2), dim=1).sum()
        orthogonal_loss = torch.pow(torch.matmul(
            self.weight, self.weight.t()) - self.I, 2).sum(dim=1).sum(dim=0)
...
class TransEdge(torch.nn.Module):
    def __init__(self, p, feat_drop, dim, mode, params):
        ...
        self.mode = mode
        if self.mode == "cc":
            self.mlp_1 = MLP(act=torch.tanh, hiddens=[
                             2*dim, dim], l2_norm=True)
            self.mlp_2 = MLP(act=torch.tanh, hiddens=[
                             2*dim, dim], l2_norm=True)
            self.mlp_3 = MLP(act=torch.tanh, hiddens=[
                             2*dim, dim], l2_norm=True)
        elif self.mode == "cp":
...
class TransR(torch.nn.Module):
    ...
    def forward(self, e1, rM, e2):
        ...
        hr = torch.matmul(e1.view(e1.size(0), 1, e1.size(1)),
                          M_r).view(e1.size(0), -1)
        tr = torch.matmul(e2.view(e2.size(0), 1, e2.size(1)),
                          M_r).view(e2.size(0), -1)
...
class HAKE(torch.nn.Module):
    ...
    def forward(self, e1, r, e2):
        ...
        r_score = mod_head * (mod_relation + bias_relation) - \
            mod_tail * (1 - bias_relation)
        phase_score = torch.sum(
            torch.abs(torch.sin(phase_score / 2)), dim=1) * self.phase_weight
