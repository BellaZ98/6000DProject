...
def multi_typed_sampling(pos, triples, ills, ids, k, params, thread=10):
    ...
    if len(pos[0]) == 2:    # triple: 1:k
        raise NotImplementedError(
            "typed_sampling is not supported in ills sampling")
    triples = set(triples)
    r_hs_dict, r_ts_dict = {}, {}
    ...
    for task in tasks:
        reses.append(pool.apply_async(
            multi_cal_neg, (pos, task, triples, r_hs_dict, r_ts_dict, ids, k)))
    pool.close()
    pool.join()
...
def typed_sampling(pos, triples, ills, ids, k, params):
    ...
    neg_part = multi_cal_neg(
        pos, tasks[0], triples, r_hs_dict, r_ts_dict, ids, k)
    neg = []
    for part in neg_part:
        neg.extend(part)
...
def nearest_neighbor_sampling(pos, triples, ills, ids, k, params):
    ...
    if len(pos[0]) == 3:    # triple: 1:k
        ...
        distance = [- sim(emb[sorted_id[0]], emb[sorted_id[0]], metric=metric, normalize=False, csls_k=0), -
                    sim(emb[sorted_id[1]], emb[sorted_id[1]], metric=metric, normalize=False, csls_k=0)]
        cache_dict = {}
        neg = []
        triples = set(triples)
        for _ in range(k):
                ...
                while True:
                    ...
                    if choice:
                        if h not in cache_dict:
                            # descending=False
                            indices = np.argsort(
                                distance[base_h][sorted_id[base_h].index(h), :])
                            cache_dict[h] = np.array(sorted_id[base_h])[
                                indices[1:]].tolist()
                        ...
                        if t not in cache_dict:
                            # descending=False
                            indices = np.argsort(
                                distance[base_t][sorted_id[base_t].index(t), :])
                            cache_dict[t] = np.array(sorted_id[base_t])[
                                indices[1:]].tolist()
        ...
        neg_left = []
        distance = - sim(emb[pos[:, 0]], emb[pos[:, 0]],
                         metric=metric, normalize=False, csls_k=0)
        ...
        neg_right = []
        distance = - sim(emb[pos[:, 1]], emb[pos[:, 1]],
                         metric=metric, normalize=False, csls_k=0)
        for idx in range(len(pos)):
            indices = np.argsort(distance[idx, :])  # descending=False
            neg_right.append(pos[:, 1][indices[1: k+1]])
        neg_right = np.stack(neg_right, axis=1).reshape(-1, 1)
        neg_left = np.concatenate((neg_left, np.tile(pos, (k, 1))[
                                  :, 1].reshape(-1, 1)), axis=1).tolist()
        neg_right = np.concatenate(
            (np.tile(pos, (k, 1))[:, 0].reshape(-1, 1), neg_right), axis=1).tolist()
...
if __name__ == '__main__':
    ...
    triples = np.random.randint(1, 9, size=(5, 3)).tolist(
    ) + np.random.randint(11, 19, size=(5, 3)).tolist()
