...
class AlignmentData:
    ...
    def __init__(self, data_dir="data/DBP15K/ja_en", rate=0.3, share=False, swap=False, val=0.0, with_r=False):
        ...
        self.rate = rate
        self.val = val
        self.ins2id_dict, self.id2ins_dict, [
            self.kg1_ins_ids, self.kg2_ins_ids] = self.load_dict(data_dir + "/ent_ids_", file_num=2)
        self.rel2id_dict, self.id2rel_dict, [
            self.kg1_rel_ids, self.kg2_rel_ids] = self.load_dict(data_dir + "/rel_ids_", file_num=2)
        ...
        self.ill_train_idx, self.ill_val_idx, self.ill_test_idx = np.array(self.ill_idx[:int(len(self.ill_idx) // 1 * rate)], dtype=np.int32), np.array(self.ill_idx[int(len(
            self.ill_idx) // 1 * rate): int(len(self.ill_idx) // 1 * (rate+val))], dtype=np.int32), np.array(self.ill_idx[int(len(self.ill_idx) // 1 * (rate+val)):], dtype=np.int32)
        self.ins_G_edges_idx, self.ins_G_values_idx, self.r_ij_idx = self.gen_sparse_graph_from_triples(
            self.triple_idx, self.ins_num, with_r)

        assert (share != swap or (share == False and swap == False))
        if share:
            self.triple_idx = self.share(
                self.triple_idx, self.ill_train_idx)   # 1 -> 2:base
            self.kg1_ins_ids = (
                self.kg1_ins_ids - set(self.ill_train_idx[:, 0])) | set(self.ill_train_idx[:, 1])
            self.ill_train_idx = []
        if swap:
    ...
    def load_dict(self, data_dir, file_num=2):
        ...
        for file_name in file_names:
            with open(file_name, "r", encoding="utf-8") as f:
                ...
                what2id = {**what2id, **
                           dict([[i[1], int(i[0])] for i in data])}
                id2what = {**id2what, **
                           dict([[int(i[0]), i[1]] for i in data])}
